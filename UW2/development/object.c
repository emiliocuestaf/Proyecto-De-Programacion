...
struct _Object{
    ...
    int locat; // If -1 the player owns the object, otherwise the number is the space where the objects is. -2=not defined -3=owned by a npc
    int coord[2]; //(-1,-1)=not defined
    int miss[2]; //Mission and phase
    char* descr;
...
/* Mission */
int Object_get_mission(Object * w){
    assert(w != NULL);
    return w->miss[0];
}
Status Object_set_mission(Object * w, int miss){
    assert(w != NULL);
    assert(miss >= 0);
    w->miss[0] = miss;
    return OK;
...
/* Phase */
int Object_get_phase(Object * w){
    assert(w != NULL);
    return w->miss[1];
}
Status Object_set_phase(Object * w, int phase){
    assert(w != NULL);
    assert(phase >= 0);
    w->miss[1] = phase;
    return OK;
...
Object* Object_create(char*name, int id, int location, int cx, int cy, char* descr, 
    ...
    Object_set_mission(o, miss[0]);
    Object_set_phase(o, miss[1]);
